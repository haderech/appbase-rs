...
impl<'a> Options<'a> {
		...
		Options {
			app: Mutex::new(Some(
				clap::App::new(s)
					...
					.arg(
						clap::Arg::new("app::plugin")
							.long("plugin")
							.takes_value(true)
							.multiple_occurrences(true),
					)
					.arg(
						clap::Arg::new("app::channel-capacity")
							.long("channel-capacity")
							.takes_value(true),
					)
					.arg(
						clap::Arg::new("app::worker-threads")
							.long("worker-threads")
							.takes_value(true),
					)
					.arg(
						clap::Arg::new("app::max-blocking-threads")
	...
	pub fn parse(&self) {
		...
		if let Some(config_dir) = self.value_of("config-dir") {
			...
			pathbuf = directories::BaseDirs::new()
				.unwrap()
				.config_dir()
				.join(Path::new(&crate::util::current_exe()));
	...
	pub fn value_of(&self, id: &str) -> Option<String> {
		...
		self.value_from_toml(id)
			.map(|value| value.as_str().map(|s| String::from(s)))
			.flatten()
	}

	pub fn value_of_t<R>(&self, id: &str) -> Option<R>
	where
		R: std::str::FromStr + serde::Deserialize<'a>,
		<R as std::str::FromStr>::Err: std::fmt::Display,
	{
		...
		self.value_from_toml(id)
			.map(|value| value.clone().try_into::<R>().ok())
			.flatten()
	}
	...
	pub fn values_of(&self, id: &str) -> Option<Vec<String>> {
		...
		self.value_from_toml(id).map(|value| {
			value
				.as_array()
				.unwrap()
				.iter()
	...
	pub fn values_of_t<R>(&self, id: &str) -> Option<Vec<R>>
	where
		R: std::str::FromStr + serde::Deserialize<'a>,
		<R as std::str::FromStr>::Err: std::fmt::Display,
	{
		...
		self.value_from_toml(id).map(|value| {
			value
				...
				.unwrap()
				.iter()
				.map(|x| x.clone().try_into::<R>().unwrap())
				.collect()
		})
	}
